...
export default function DeliveriesPage() {
  ...
  const entregas = [
    {
      ...
      fornecedor: "Concretos São Paulo Ltda",
      material: "Concreto Usinado FCK 30",
      quantidade: "20m³",
      status: "entregue",
      ...
      notaFiscal: "NF-45678",
      observacoes: "Entrega conforme programado. Material em perfeito estado."
    },
    {
      id: 2,
      codigo: "ENT-2024-002",
      projeto: "Edifício Vila Madalena",
      fornecedor: "Cerâmica Paulista",
      material: "Tijolos Cerâmicos 8 furos",
      quantidade: "10000 un",
      status: "em_transito",
      dataPrevista: "22/12/2024",
      dataEntrega: "-",
      horario: "14:00",
      responsavelRecebimento: "João Pereira",
      notaFiscal: "-",
      observacoes: "Saiu para entrega às 11h. Previsão de chegada às 14h."
    },
    {
      id: 3,
      codigo: "ENT-2024-003",
      projeto: "Centro Comercial Brooklin",
      fornecedor: "Elétrica Industrial",
      material: "Fiação Elétrica 2.5mm",
      quantidade: "500m",
      status: "agendado",
      dataPrevista: "23/12/2024",
      dataEntrega: "-",
      horario: "09:00",
      responsavelRecebimento: "Roberto Lima",
      notaFiscal: "-",
      observacoes: "Agendado para manhã. Preparar área de descarregamento."
    },
    {
      id: 4,
      codigo: "ENT-2024-004",
      ...
      dataEntrega: "-",
      horario: "10:30",
      responsavelRecebimento: "Paulo Oliveira",
      notaFiscal: "-",
      observacoes: "Entrega programada para após feriado."
    },
    {
      id: 5,
      codigo: "ENT-2024-005",
      projeto: "Condomínio Alphaville",
      fornecedor: "Paisagismo Verde Ltda",
      material: "Mudas de Árvores Nativas",
      quantidade: "30 un",
      status: "entregue",
      dataPrevista: "18/12/2024",
      dataEntrega: "18/12/2024",
      horario: "07:30",
      responsavelRecebimento: "Ana Costa",
      notaFiscal: "NF-12345",
      observacoes: "Mudas em excelente estado. Já plantadas."
    },
    {
      id: 6,
      codigo: "ENT-2024-006",
      projeto: "Edifício Vila Madalena",
      fornecedor: "Cimento Forte SA",
      material: "Cimento CP-II 50kg",
      quantidade: "200 sacas",
      status: "atrasado",
      dataPrevista: "19/12/2024",
      dataEntrega: "-",
      horario: "13:00",
      responsavelRecebimento: "Carlos Silva",
      notaFiscal: "-",
      observacoes: "Atraso por problema logístico do fornecedor. Reagendado para 26/12."
    }
  ];
  ...
  const filteredEntregas = entregas.filter(ent => {
    const matchesSearch = 
      ent.codigo.toLowerCase().includes(searchTerm.toLowerCase()) ||
      ent.material.toLowerCase().includes(searchTerm.toLowerCase()) ||
      ent.projeto.toLowerCase().includes(searchTerm.toLowerCase()) ||
      ent.fornecedor.toLowerCase().includes(searchTerm.toLowerCase());
    
    const matchesStatus = filterStatus === "todos" || ent.status === filterStatus;
      ...
      <div className="flex-1 flex flex-col h-screen overflow-hidden">
        <header className="bg-white border-b border-gray-200 px-4 py-4 lg:px-6 flex-shrink-0">
          <div className="flex items-center gap-3 mb-4">
            ...
            <div className="flex-1">
              <h1 className="text-xl font-bold text-gray-900">Entrega de Material</h1>
              <p className="text-xs text-gray-500">{filteredEntregas.length} entregas</p>
            </div>
          ...
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 mb-3">
            <div className="bg-blue-50 rounded-lg p-3 border border-blue-200">
              <p className="text-xs text-blue-600 mb-1">Agendados</p>
              <p className="text-xl font-bold text-blue-900">{stats.agendados}</p>
            </div>
            <div className="bg-yellow-50 rounded-lg p-3 border border-yellow-200">
              <p className="text-xs text-yellow-600 mb-1">Em Trânsito</p>
              <p className="text-xl font-bold text-yellow-900">{stats.emTransito}</p>
            </div>
            <div className="bg-green-50 rounded-lg p-3 border border-green-200">
              <p className="text-xs text-green-600 mb-1">Entregues</p>
              <p className="text-xl font-bold text-green-900">{stats.entregues}</p>
            </div>
            <div className="bg-red-50 rounded-lg p-3 border border-red-200">
              <p className="text-xs text-red-600 mb-1">Atrasados</p>
              <p className="text-xl font-bold text-red-900">{stats.atrasados}</p>
            </div>
          </div>

          <div className="relative mb-3">
            <Search className="absolute left-3 top-1/2 -translate-y-1/2 w-5 h-5 text-gray-400" />
            <input type="text" placeholder="Buscar entregas..." value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)}
        ...
        <div className="flex-1 overflow-y-auto px-4 py-4 lg:px-6 space-y-4">
          {filteredEntregas.map((ent) => {
            const statusConfig = getStatusConfig(ent.status);
            const StatusIcon = statusConfig.icon;

            return (
              <div key={ent.id} className={`bg-white rounded-2xl p-5 border shadow-sm hover:shadow-md transition-all ${
                ent.status === "atrasado" ? "border-red-200" : "border-gray-200 hover:border-blue-200"
              }`}>
                <div className="flex items-start gap-4 mb-4">
                  ...
                  <div className="flex-1">
                    ...
                    <h3 className="text-base font-bold text-gray-900 mb-1">{ent.material}</h3>
                    <p className="text-sm text-blue-600 font-medium mb-1">{ent.fornecedor}</p>
                    <p className="text-xs text-gray-500">{ent.projeto}</p>
                  </div>
                </div>
                ...
                <div className="grid grid-cols-2 gap-3 mb-4">
                  <div className="bg-gray-50 rounded-lg p-3 border border-gray-100">
                    ...
                    <p className="text-sm font-bold text-gray-900">{ent.quantidade}</p>
                  </div>
                  <div className="bg-gray-50 rounded-lg p-3 border border-gray-100">
                    <p className="text-xs text-gray-500 mb-1">Horário</p>
                    <p className="text-sm font-bold text-gray-900">{ent.horario}</p>
                  </div>
                </div>
                ...
                <div className="space-y-2 mb-4">
                  <div className="flex items-center gap-2 text-sm">
                    ...
                    <span className="text-xs text-gray-600">
                      {ent.status === "entregue" ? `Entregue em: ${ent.dataEntrega}` : `Previsto: ${ent.dataPrevista}`}
                    </span>
                  </div>
                  <div className="flex items-center gap-2 text-sm">
                    <Building2 className="w-4 h-4 text-gray-400" />
                    <span className="text-xs text-gray-600">Responsável: {ent.responsavelRecebimento}</span>
                  </div>
                  {ent.notaFiscal !== "-" && (
                    <div className="flex items-center gap-2 text-sm">
                      <Package className="w-4 h-4 text-gray-400" />
                      <span className="text-xs text-gray-600">NF: {ent.notaFiscal}</span>
                    </div>
                  )}
                  <div className={`border rounded-lg p-2 ${
                    ent.status === "atrasado" ? "bg-red-50 border-red-100" : "bg-blue-50 border-blue-100"
                  }`}>
                    <p className={`text-xs ${ent.status === "atrasado" ? "text-red-700" : "text-blue-700"}`}>
                      {ent.observacoes}
                    </p>
                  </div>
                </div>
